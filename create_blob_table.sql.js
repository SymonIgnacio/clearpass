...
async function createTableAndInsert() {
  ...
  try {
    ...
    // Insert sample data for testing (separate call)
    ...
    console.log(`✅ Inserted ${insertResult.affectedRows} test record(s)`);
    ...
    // Show sample data
    const [sampleRows] = await connection.execute('SELECT id, request_id, user_id, proof_type, status, submitted_at FROM resident_verification_requests LIMIT 5');
