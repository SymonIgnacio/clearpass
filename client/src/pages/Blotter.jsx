...
const Blotter = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      await Promise.all([fetchBlotterCases(), fetchResidents(), fetchSitios()]);
      setLoading(false);
    };
    loadData();
  }, []);
  ...
  const handleEdit = case_ => {
    ...
    setWizardData({
      ...
      dateTimeIncident: case_.DateTime_Incident
        ? new Date(case_.DateTime_Incident).toISOString().slice(0, 16)
        : '',
    });
    ...
    try {
      ...
      if (response.ok) {
        ...
        const createdCaseNumber =
          created?.Case_Number || created?.case_number || created?.data?.Case_Number;
        fetchBlotterCases();
        setOpenWizard(false);
        setWizardData({
          complainantDetails: {
        ...
        setSnackbar({
          open: true,
          message: 'Blotter case created successfully! PDF extract will be generated.',
          severity: 'success',
        });
  ...
  const handleIssueSummons = async caseData => {
    try {
      ...
      if (response.ok) {
        ...
        setSnackbar({
          open: true,
          message: 'Summons issued successfully! KP Form #9 generated.',
          severity: 'success',
        });
      }
    } catch (error) {
  ...
  const handleResolution = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setSnackbar({
          open: true,
          message: `Case ${newStatus.toLowerCase()}.`,
          severity: 'success',
        });
      }
    } catch (error) {
  ...
  return (
    <Box>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        ...
                        {(() => {
                          const caseStatus = (case_?.Status ?? case_?.status ?? '').toLowerCase();
                          return (
                            caseStatus === 'scheduled for mediation' || caseStatus === 'ongoing'
                          );
                        })() && (
                          <Button
                            size='small'
      ...
      >
        <DialogTitle>
          {isEditing ? 'Edit Complaint' : 'File a Complaint - Katarungang Pambarangay'}
        </DialogTitle>
        <DialogContent>
                        ...
                        if (complainantData) {
                          setWizardData({
                            ...
                            complainantDetails: {
                              ...
                              address:
                                complainantData.address || wizardData.complainantDetails.address,
                              contact:
                                complainantData.mobile || wizardData.complainantDetails.contact,
                        ...
                        if (respondentData) {
                          setWizardData({
                            ...
                            respondentDetails: {
                              ...
                              address:
                                respondentData.address || wizardData.respondentDetails.address,
                              contact:
                                respondentData.mobile || wizardData.respondentDetails.contact,
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Autocomplete
                ...
                onChange={(event, newValue) => {
                  if (newValue && newValue.label === 'Others') {
                    ...
                    setWizardData({
                      ...wizardData,
                      incidentType: newValue ? newValue.label : '',
                      customIncidentType: '',
                    });
                  }
                }}
                renderInput={params => (
                  <TextField {...params} label='Incident Type' required fullWidth />
                )}
                renderGroup={params => (
                  <li key={params.key}>
                    <ListSubheader
              ...
              {wizardData.incidentType === 'Others' && (
                <TextField
                  ...
                  onChange={e =>
                    setWizardData({ ...wizardData, customIncidentType: e.target.value })
                  }
                  required
                  sx={{ width: '100%' }}
                />
      ...
      {/* Summons Dialog */}
      <Dialog
        open={openSummonsDialog}
        onClose={() => setOpenSummonsDialog(false)}
        maxWidth='sm'
        fullWidth
      >
        ...
        <DialogContent>
          ...
          <Typography variant='body2' color='text.secondary' sx={{ mb: 3 }}>
            This will generate a KP Form #9 summons inviting the respondent to the Barangay Hall for
            mediation.
          </Typography>
          <Alert severity='info'>A hearing will be scheduled for 7 days from now.</Alert>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setOpenSummonsDialog(false)}>Cancel</Button>
          <Button
            onClick={() => handleIssueSummons(selectedCase)}
            variant='contained'
            startIcon={<Schedule />}
          >
            Issue Summons
      ...
      >
          ...
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <TextField
                ...
                onChange={e =>
                  setResolutionData({ ...resolutionData, hearingDate: e.target.value })
                }
                InputLabelProps={{ shrink: true }}
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
                ...
                onChange={e =>
                  setResolutionData({ ...resolutionData, outcomeNotes: e.target.value })
                }
        ...
        <DialogActions sx={{ p: 2.5 }}>
          ...
          <Button onClick={handleResolution} variant='contained' disabled={!resolutionData.outcome}>
            Confirm & Submit
          </Button>
        </DialogActions>
      </Dialog>
