...
const DocumentsDashboard = () => {
  ...
  const [certificateFormData, setCertificateFormData] = useState({
    ...
    manual_valid_until: '',
    // manual_control_number removed to allow auto-generation
  ...
  useEffect(() => {
    if (selectedCertificateTemplate) {
      ...
      // Only reset if we actually have placeholders to clear
      if (extractedPlaceholders.length > 0) {
        setExtractedPlaceholders([]);
        setDynamicFields({});
      }
    }
    // Removed extractedPlaceholders from dependency array to prevent infinite loop
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      const [certificatesRes, templatesRes, certTypesRes, requestsRes, residentsRes, statsRes] =
        await Promise.all([
          ...
          hasManagementAccess
            ...
            : Promise.resolve({ data: [] }),
          apiRequest('residents?limit=100').catch(err => {
            return { data: [] };
          }),
          shouldFetchStats
      ...
      const statsData =
        shouldFetchStats && statsRes && statsRes.ok
          ? await statsRes.json().catch(err => {
              return null;
            })
          : null;

      const safeTemplates = Array.isArray(templateData.data)
        ? templateData.data
  ...
  const handleIssueCertificate = async () => {
    try {
      ...
      const manualType =
        certificateFormData.manual_certificate_type ||
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          if (data.request_id) {
            ...
            window.open(
              `${window.location.origin}/api/documents/requests/${data.request_id}/download`,
              '_blank'
            );
          } else {
            // Fallback for manual (legacy or if ID missing)
            window.open(
              `${window.location.origin}/api/documents/download?type=${encodeURIComponent(formData.certificate_type)}&manual=true&controlNo=${data.control_no}`,
              '_blank'
            );
          }
        }, 500);
  ...
  const handleDeleteTemplateWithFileConfirm = async () => {
    try {
      const response = await apiRequest(`templates/${confirmationAction.id}/with-file`, {
        method: 'DELETE',
      });
  ...
  const handleFileUpload = async event => {
    ...
    formData.append(
      'template_name',
      uploadFormData.template_name || file.name.replace(/\.[^/.]+$/, '')
    );
  ...
  const canViewTemplates = [ROLES.ADMIN, ROLES.CAPTAIN, ROLES.SECRETARY].includes(
    Number(user?.role)
  );
  ...
  const canProcessRequests = [ROLES.ADMIN, ROLES.SECRETARY, ROLES.CLERK].includes(
    Number(user?.role)
  );
    ...
    <Box>
                ...
                <Typography variant='h3' sx={{ fontWeight: 700, color: '#f57c00', mb: 1 }}>
                  {
                    certificates.filter(
                      cert =>
                        new Date(cert.date_issued || cert.issued_date).getMonth() ===
                        new Date().getMonth()
                    ).length
                  }
                </Typography>
      ...
      <Paper sx={{ borderRadius: 3, overflow: 'hidden' }}>
        ...
        >
          ...
          {canProcessRequests && (
            <Tab icon={<PendingActions />} label='Certificate Requests' iconPosition='start' />
          )}

          {/* Certificate History - Visible to All Staff */}
          <Tab icon={<Description />} label='Certificate History' iconPosition='start' />

          {canViewTemplates && (
            <Tab icon={<Settings />} label='Document Templates' iconPosition='start' />
          )}

          {canViewTypes && (
            <Tab icon={<Assignment />} label='Certificate Types' iconPosition='start' />
          )}
        </Tabs>
                      ...
                      <CardContent>
                        ...
                        <Alert severity='info' sx={{ mb: 3, borderRadius: 2 }}>
                          <Typography variant='body2'>
                            <strong>Super Admin Feature:</strong> Create certificates with custom
                            information. All fields are manually entered for complete control over
                            certificate content.
                          </Typography>
                        </Alert>
                        ...
                        <Box sx={{ mb: 3 }}>
                              ...
                              {templates
                                ...
                                .map(template => (
                                  <MenuItem key={template.id} value={template.id}>
                                    ...
                                    {documentTypes.find(dt => dt.value === template.document_type)
                                      ?.label || template.document_type}
                                    )
                                  </MenuItem>
                                ))}
                            </Select>
                          ...
                          {selectedCertificateTemplate && (
                            <Alert severity='success' sx={{ borderRadius: 2 }}>
                              <Typography variant='body2'>
                                ...
                                {
                                  templates.find(t => t.id === selectedCertificateTemplate)
                                    ?.template_name
                                }
                                <br />
                          ...
                          {!selectedCertificateTemplate && (
                            <Alert severity='warning' sx={{ borderRadius: 2 }}>
                              <Typography variant='body2'>
                                Please select a document template first before issuing a
                                certificate.
                              </Typography>
                            </Alert>
                          )}
                        ...
                        <Grid container spacing={2}>
                          <Grid item xs={12} md={6}>
                            ...
                            <TextField
                              ...
                              sx={{
                                mt: 1,
                                display: certificateFormData.manual_resident_name
                                  ? 'block'
                                  : 'none',
                              }}
                          ...
                          {extractedPlaceholders.length > 0 && (
                            <Grid item xs={12}>
                              <Typography
                                variant='subtitle2'
                                sx={{ mb: 2, mt: 1, color: 'primary.main' }}
                              >
                          ...
                          {certificates
                            ...
                            .map(cert => (
                              <TableRow key={cert.id || cert.control_no}>
                                ...
                                <TableCell>
                                  {new Date(
                                    cert.date_issued || cert.issued_date
                                  ).toLocaleDateString()}
                                </TableCell>
                                <TableCell>
                                  <IconButton
                                    ...
                                    onClick={() =>
                                      window.open(
                                        `/api/documents/requests/${cert.request_id || cert.id}/download`,
                                        '_blank'
                                      )
                                    }
                                  >
          ...
          {certificateFormData.use_manual_input ? (
            ...
            <Box sx={{ pt: 2 }}>
              <Typography variant='body1' gutterBottom>
                ...
                {residents.find(r => r.Resident_ID === certificateFormData.resident_id)?.First_Name}{' '}
                {residents.find(r => r.Resident_ID === certificateFormData.resident_id)?.Last_Name}
              </Typography>
              <Typography variant='body1' gutterBottom>
...
const CertificateTypesManagement = ({
  user,
  certificateTypes,
  loadAllData,
  canManage,
  ...
  const handleDeleteTypeConfirm = async () => {
    try {
      const response = await apiRequest(`certificate-types/${confirmationAction.id}`, {
        method: 'DELETE',
      });
                ...
                {certificateTypes.map(type => (
                  <TableRow key={type.id} hover>
                    ...
                    <TableCell>
                      ...
                      {!canManage && <Chip label='Read Only' size='small' variant='outlined' />}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
...
const CertificateRequestsManagement = ({
  user,
  requests,
  loadAllData,
  canManage,
  ...
  const handleViewRequest = async request => {
    ...
    try {
      ...
      const [frontRes, backRes] = await Promise.all([
        apiRequest(`certificate-requests/${request.request_id}/attachment/front`, {
          responseType: 'blob',
        }),
        apiRequest(`certificate-requests/${request.request_id}/attachment/back`, {
          responseType: 'blob',
        }),
      ]);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const response = await apiRequest(
        `certificate-requests/${selectedRequest.request_id}/details`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleRejectionConfirm = async reason => {
    ...
    try {
      const response = await apiRequest(`certificate-requests/${rejectionAction.id}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        showSnackbar(data.message || 'Error rejecting request', 'error');
      }
    } catch (error) {
      ...
      if (response.ok) {
        ...
        if (status === 'approved') {
          ...
          if (request) {
            ...
            setTimeout(() => {
              window.open(
                `${window.location.origin}/api/documents/requests/${requestId}/download`,
                '_blank'
              );
            }, 500);
          }
      ...
      <Dialog open={viewDialogOpen} onClose={handleCloseDialog} maxWidth='md' fullWidth>
        ...
        <DialogActions>
          ...
          {selectedRequest?.status === 'pending' && (
            <>
              <Button color='error' startIcon={<Cancel />} onClick={() => handleReject()}>
                Reject
              </Button>
              <Button
                color='success'
  ...
  return (
    <Dialog open={true} onClose={onClose} maxWidth='lg' fullWidth>
      ...
      <DialogContent>
        <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Alert severity='info' sx={{ borderRadius: 2 }}>
            <Typography variant='body2'>
              <strong>Note:</strong> Required data items will be displayed to residents when
      ...
      <DialogActions>
        ...
        <Button onClick={onSubmit} variant='contained' sx={{ backgroundColor: '#1DB954' }}>
