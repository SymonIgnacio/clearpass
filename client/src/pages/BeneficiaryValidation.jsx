...
const BeneficiaryValidation = () => {
  ...
  const handleAction = async (id, action, reason = '') => {
    try {
      ...
      if (response.ok) {
        fetchBeneficiaries();
        notify(
          `Beneficiary ${action === 'approve' ? 'approved' : 'rejected'} successfully`,
          'success'
        );
      } else {
  ...
  const openApprovalModal = id => {
    setConfirmationAction({
      ...
      message:
        'Are you sure you want to approve this beneficiary? This will confirm their status in the system.',
      icon: 'success',
    });
  ...
  return (
    <Box>
      ...
      <Dialog open={viewDocsOpen} onClose={() => setViewDocsOpen(false)} maxWidth='md' fullWidth>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          openFileFromEndpoint(
                            `residents/${doc.resident_id}/documents/${doc.id}/download`,
                            doc.file_name
                          )
                        }
                      >
