...
const CommunityEvents = () => {
  ...
  const fetchResidents = async () => {
    try {
      const response = await apiRequest('residents', { params: { limit: 50 } }).catch(() => {
        return { data: { data: [] } };
      });
  ...
  const fetchSitios = async () => {
    try {
      const response = await apiRequest('sitios').catch(() => {
        return [];
      });
      const data = response.json
        ? await response.json().catch(() => {
  ...
  return (
    <Box>
              ...
              <CardContent>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  <Button
                    size='small'
                    variant='outlined'
                    onClick={() => setParticipantsDialog(event)}
                  >
                    <PersonAdd sx={{ mr: 1 }} />
                    Add Participants
                  </Button>
      ...
      <Dialog
        ...
        onClose={() => setParticipantsDialog(null)}
        maxWidth='md'
        fullWidth
      >
                ...
                <CardContent sx={{ py: 1 }}>
                  ...
                  >
                    ...
                    <Button
                      ...
                      variant='outlined'
                      onClick={() =>
                        handleAddParticipant(participantsDialog.id, resident.Resident_ID)
                      }
                    >
                      Add
                    </Button>
      ...
      <Dialog open={!!smsDialog} onClose={() => setSmsDialog(null)} maxWidth='sm' fullWidth>
        ...
        <DialogActions>
          ...
          <Button onClick={handleConfirmSendSMS} variant='contained' disabled={!smsMessage.trim()}>
            Send SMS
          </Button>
        </DialogActions>
      </Dialog>
