...
const TemplateUploadWizard = ({ open, onClose, onSuccess }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formData.append('document_type', documentType === 'custom' ? templateName.toLowerCase().replace(/\s+/g, '_') : documentType);
        ...
        return (
          <Box sx={{ mt: 2 }}>
            <Typography variant="body2" sx={{ mb: 2 }}>
              We found <b>{placeholders.length}</b> placeholders in your document. Configure how they should be filled.
            </Typography>
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        ...
        {error && (
          <Box sx={{ mb: 2, p: 2, bgcolor: '#fff0f0', color: '#d32f2f', borderRadius: 1, display: 'flex', alignItems: 'center' }}>
            <Warning sx={{ mr: 1 }} />
            {error}
          </Box>
        )}
