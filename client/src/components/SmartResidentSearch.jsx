...
const SmartResidentSearch = ({ value, onChange, label = "Search Resident", required = false, error = false, helperText = "" }) => {
  ...
  const fetchResidents = useCallback(async (search = '') => {
    try {
      ...
      setResidents(residentsList.map(resident => ({
        ...
        address: resident.sitio_name ? `${resident.sitio_name}, Household ${resident.Household_ID}` : `Household ${resident.Household_ID}`,
