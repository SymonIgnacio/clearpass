...
async function generateBlotterData() {
  ...
  for (let i = 0; i < 500; i++) {
    ...
    const complainantDetails = {
      ...
      address: `Block ${Math.floor(Math.random() * 20) + 1}, Lot ${
    ...
    if (Math.random() > 0.3) {
      respondentDetails = {
        ...
        address: `Block ${Math.floor(Math.random() * 20) + 1}, Lot ${
          Math.floor(Math.random() * 50) + 1
        }, ${sitios[Math.floor(Math.random() * sitios.length)]}`,
        contact: `09${Math.floor(Math.random() * 900000000) + 100000000}`,
        age: Math.floor(Math.random() * 60) + 18,
      };
    }
    ...
    const narrative =
      typeNarratives[Math.floor(Math.random() * typeNarratives.length)];
    ...
    blotterData.push({
      ...
      narrative: narrative,
      date_time_incident: incidentDate
        .toISOString()
        .slice(0, 19)
        .replace("T", " "),
...
async function generateResidentData(db) {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    const householdNumber = `HH-${Date.now().toString().substr(-6)}-${i}`;

    households.push({
      Household_ID: householdId,
      Household_Number: householdNumber,
      Sitio_ID: sitioMap[sitio],
      Street_Address: `Block ${Math.floor(Math.random() * 20) + 1}, Lot ${
        Math.floor(Math.random() * 50) + 1
      }, ${sitio}`,
  ...
  for (let i = 0; i < households.length; i += 50) {
    const batch = households.slice(i, i + 50);
    ...
    try {
      await db.execute(
        ...
        INSERT INTO households (
          Household_ID, Household_Number, Sitio_ID, 
          Street_Address, Total_Members, Household_Type
        ) VALUES ${placeholders}
      ...
      console.error("Error inserting household batch:", error.message);
    }
  }
...
async function generateCertificateData(db) {
  ...
  const [residents] = await db.execute(
    "SELECT Resident_ID FROM residents LIMIT 100"
  );
  ...
  for (let i = 0; i < 150; i++) {
    const residentId =
      residents[Math.floor(Math.random() * residents.length)].Resident_ID;
    const certType =
      certificateTypes[Math.floor(Math.random() * certificateTypes.length)];
  ...
  for (let i = 0; i < 200; i++) {
    ...
    const userMessage =
      messageTemplates[intent][
        Math.floor(Math.random() * messageTemplates[intent].length)
      ];
    const botResponse =
      responseTemplates[intent][
        Math.floor(Math.random() * responseTemplates[intent].length)
      ];
...
async function runComprehensiveDataGeneration() {
  ...
  try {
    ...
    const [existingBlotter] = await db.execute(
      "SELECT COUNT(*) as count FROM blotter"
    );
    ...
    for (const record of blotterData) {
      try {
        await db.execute(
          ...
          [
            record.case_number,
            record.complainant_details,
            record.respondent_details,
            record.incident_type,
            record.narrative,
            record.date_time_incident,
            record.location_sitio,
            record.status,
            record.created_at,
          ]
        );
        ...
        console.error(
          `Error inserting blotter ${record.case_number}:`,
          error.message
        );
      }
    }
    ...
    if (existingResidents[0].count < 250) {
    ...
    const [existingCertificates] = await db.execute(
      "SELECT COUNT(*) as count FROM certificates_log"
    );
    ...
    const [existingPrograms] = await db.execute(
      "SELECT COUNT(*) as count FROM community_programs"
    );
    ...
    try {
      const [existingChatbot] = await db.execute(
        "SELECT COUNT(*) as count FROM ai_chatbot_conversations"
      );
      console.log(
        `Existing chatbot conversations: ${existingChatbot[0].count}`
      );
      ...
      console.log(
        "⚠️ AI chatbot tables not found, skipping chatbot data generation"
      );
    }
