...
async function repopulateUsers(trx) {
  ...
  const adminRole = roles.find(
    r => r.role_name === 'admin' || r.role_name === 'superadmin' || r.role_name === 'IT Admin'
  );

  if (!adminRole) {
    console.warn(
      '   ⚠️ WARNING: No admin/superadmin/IT Admin role found in roles table. Cannot create admin user.'
    );
    return;
  }
...
async function repopulateTestData(trx) {
  ...
  const existingHH = await trx('households')
    .whereIn(
      'Household_ID',
      householdsToInsert.map(h => h.Household_ID)
    )
    .pluck('Household_ID');
  ...
  const existingRes = await trx('residents')
    .whereIn(
      'Resident_ID',
      residentsToInsert.map(r => r.Resident_ID)
    )
    .pluck('Resident_ID');
  ...
  for (const [hhId, headId] of householdMap) {
    ...
    if (newHH.find(h => h.Household_ID === hhId)) {
      await trx('households').where('Household_ID', hhId).update({
