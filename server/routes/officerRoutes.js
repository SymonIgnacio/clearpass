...
module.exports = (db) => {
  // Officer dashboard
  router.get('/dashboard', verifyToken, checkRole(['blotter_officer']), asyncHandler(async (req, res) => {
    const [pendingCases] = await db.execute('SELECT COUNT(*) as total FROM blotter WHERE Status = "Pending"');
    const [ongoingCases] = await db.execute('SELECT COUNT(*) as total FROM blotter WHERE Status = "Ongoing"');
    const [resolvedToday] = await db.execute('SELECT COUNT(*) as total FROM blotter WHERE Status = "Resolved" AND DATE(updated_at) = CURDATE()');
  ...
  // Case management
  router.get('/cases', verifyToken, checkRole(['blotter_officer']), asyncHandler(async (req, res) => {
  ...
  // Resolve case
  router.put('/cases/:caseNumber/resolve', verifyToken, checkRole(['blotter_officer']), asyncHandler(async (req, res) => {
    ...
    await db.execute(`
      ...
      WHERE Case_Number = ?
    `, [resolution_type || 'Amicably Settled', resolution_notes, resolution_type, req.user.id, caseNumber]);
    
    res.json({ message: 'Case resolved successfully' });
  }));

  // Update case status
  router.put('/cases/:caseNumber/status', verifyToken, checkRole(['blotter_officer']), asyncHandler(async (req, res) => {
    const { caseNumber } = req.params;
  ...
  router.get('/ai-analytics', verifyToken, checkRole(['blotter_officer']), asyncHandler(async (req, res) => {
  ...
  // Reports
  router.get('/reports', verifyToken, checkRole(['blotter_officer']), asyncHandler(async (req, res) => {
