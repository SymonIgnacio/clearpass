...
module.exports = db => {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      const { name, validity_days, description, purpose, when_needed, required_data, is_active } =
        req.body;
      ...
      await db.execute(
        ...
        (name, validity_days, description, purpose, when_needed, required_data, is_active, created_at, updated_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        [
          name,
          validity_days || 365,
  ...
  router.put(
    ...
    asyncHandler(async (req, res) => {
      ...
      await db.execute(
        ...
        SET name = ?, validity_days = ?, description = ?, purpose = ?, when_needed = ?, required_data = ?, is_active = ?, updated_at = NOW() 
        WHERE id = ?`,
        [
          name,
          validity_days || 365,
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      const [certs] = await db.execute(
        'SELECT COUNT(*) as count FROM certificates_log WHERE certificate_type = (SELECT name FROM certificate_types WHERE id = ?)',
        [id]
      );
      if (certs[0].count > 0) {
        return res.status(400).json({
          error:
            'Cannot delete: Certificates of this type have already been issued. Deactivate it instead.',
        });
      }
