...
module.exports = db => {
  ...
  router.post(
    '/priority',
    verifyToken,
    checkRole(['admin', 'secretary', 'clerk', 'captain']),
    asyncHandler(async (req, res) => {
      if (!AI_SERVICE_ENABLED) {
        return res.status(503).json({
          success: false,
      ...
      try {
        ...
        const response = await aiServiceAxios.post('/api/calculate-priority', req.body, {
          timeout: 10000,
        });
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      `);
        ...
        const response = await aiServiceAxios.post(
          '/suggest-patrol',
          {
            blotter_data: rows,
          },
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        const response = await aiServiceAxios.get('/analytics/general', {
          timeout: 5000,
        });
