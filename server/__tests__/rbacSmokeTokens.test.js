...
describe('RBAC smoke via signed JWT', () => {
  ...
  const makeApp = () => {
    ...
    app.get('/admin-only', verifyToken, checkRole([ROLES.ADMIN]), (req, res) =>
      res.json({ ok: true })
    );
    app.get(
      '/staff',
    ...
    app.get('/resident-only', verifyToken, checkRole([ROLES.RESIDENT]), (req, res) =>
      res.json({ ok: true })
    );
    app.post('/officer-only', verifyToken, checkRole([ROLES.BLOTTER_OFFICER]), (req, res) =>
      res.json({ ok: true })
    );
    return app;
  };
  ...
  test('admin-only endpoint allows admin and denies others', async () => {
    ...
    const ok = await request(app)
      .get('/admin-only')
      .set('Authorization', `Bearer ${sign(ROLES.ADMIN)}`);
    expect(ok.status).toBe(200);
    const denied = await request(app)
      .get('/admin-only')
      .set('Authorization', `Bearer ${sign(ROLES.RESIDENT)}`);
  ...
  test('staff endpoint denies residents', async () => {
    ...
    const staffOk = await request(app)
      .get('/staff')
      .set('Authorization', `Bearer ${sign(ROLES.SECRETARY)}`);
  ...
  test('officer-only endpoint allows blotter officer only', async () => {
    ...
    const ok = await request(app)
      .post('/officer-only')
      .set('Authorization', `Bearer ${sign(ROLES.BLOTTER_OFFICER)}`);
    expect(ok.status).toBe(200);
    const denied = await request(app)
      .post('/officer-only')
      .set('Authorization', `Bearer ${sign(ROLES.CLERK)}`);
    expect(denied.status).toBe(403);
  });
});
