exports.up = function (knex) {
  return knex.schema
    ...
    .createTable('households', function (table) {
      ...
      table.enu('Household_Type', ['Nuclear', 'Extended', 'Single', 'Boarding']).defaultTo('Nuclear');
      table.timestamp('created_at').defaultTo(knex.fn.now());
    ...
    .createTable('residents', function (table) {
      ...
      table.enu('Relation_to_Head', ['Head', 'Spouse', 'Child', 'Relative', 'Boarder']).defaultTo('Head');
      ...
      table.enu('Civil_Status', ['Single', 'Married', 'Widowed', 'Separated', 'Divorced']).defaultTo('Single');
      ...
      table.enu('Residency_Status', ['Active', 'Deceased', 'Transferred Out', 'Transient']).defaultTo('Active');
    ...
    .then(() => {
      return knex.schema.alterTable('households', function (table) {
    ...
    .createTable('blotter', function (table) {
      ...
      table.enu('Location_Sitio', ['Batia Proper', 'Northville 5', 'St. Martha', 'AFP/PNP']).notNullable();
      table.enu('Status', ['Pending', 'Scheduled for Mediation', 'Amicably Settled', 'Certificate to File Action Issued', 'Dismissed', 'Ongoing']).defaultTo('Pending');
    ...
    .createTable('certificate_types', function (table) {
      table.increments('id').primary();
      table.string('name', 100).notNullable().unique();
      table.decimal('fee', 8, 2).defaultTo(0.00);
    ...
    .createTable('certificates_log', function (table) {
      ...
      table.decimal('fee_amount', 8, 2).defaultTo(0.00);
      table.timestamp('created_at').defaultTo(knex.fn.now());
    ...
    .createTable('users', function (table) {
      ...
      table.enu('role', ['admin', 'captain', 'secretary', 'clerk', 'tanod', 'resident']).notNullable();
      table.string('email', 100);
    ...
    .createTable('tanod_patrol_schedule', function (table) {
      ...
      table.enu('status', ['Scheduled', 'Active', 'Completed', 'Cancelled']).defaultTo('Scheduled');
    ...
    .createTable('community_programs', function (table) {
      ...
      table.decimal('budget_allocated', 10, 2).defaultTo(0.00);
      table.decimal('actual_cost', 10, 2).defaultTo(0.00);
